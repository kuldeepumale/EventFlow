...
interface SocialLoginProps {
  onLoginSuccess: (accessToken: string, userId: string, email: string) => void;
}
  ...
  const handleSocialLogin = async (provider: 'google' | 'facebook' | 'github') => {
    ...
    try {
      const response = await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-41f20081/auth/social-login`,
        {
          method: 'POST',
          headers: {
      ...
      if (data.url) {
        ...
        toast.info(`Please complete setup for ${provider} OAuth in Supabase dashboard`, {
          duration: 8000,
        });
  ...
  return (
    <div className="space-y-3">
      ...
      <p className="text-xs text-center text-gray-500 mt-4">
        Note: Social login requires OAuth provider setup in Supabase dashboard.
        <br />
        <a
